...
| `/loop` | `handleLoop()` | Lane-flow prompt driver only. Resolves the selected lane prompt, gathers ready tasks, interpolates it, and offers chat injection/clipboard fallback. It does not submit chat, mutate lanes, execute shell commands, or provide a daemon loop. Standard mapping: `backlog -> stage-backlog-to-planning`, `planning`/`in-progress -> stage-planning-to-review`, `review -> stage-review-to-done`; Lite mapping: `backlog -> stage-backlog-to-inprogress`, `in-progress -> stage-inprogress-to-done`. Gates apply when an agent moves a task. For file-backed autorun, use `agentKanban.startAutorun`; its controller watches meaningful workflow snapshots and stops only at fixed point or explicit stop. |
