...
2. Cleared when: every `dependsOn` is `done`, or the recorded condition is resolved (confirm it —
   don't assume).
3. Cleared → remove the satisfied labels, note how it cleared, continue via the matching driver
...
Emit a `resumed` / `still blocked` summary.
```
