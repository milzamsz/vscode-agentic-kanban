...
Finalize EVERY task whose implementation review passed: production-readiness pass, Definition of Done checklist,
release (if asked), handover, cleanup. Run rules: [conventions.md](conventions.md). Sweep mechanics: [batch-and-dependencies.md](batch-and-dependencies.md).
...
2. **Definition of Done checklist (if board policy requires it):** Confirm a `## Definition of Done` section
   exists in the task body with all items checked. Items tagged `(human)` require a human actor.
3. **Release & handover (only what the user asks):** commit/tag/package/deploy only when instructed; if on
   the default branch, branch first. Summarize in the task file: what shipped, how verified, follow-ups/limits.
4. **Finalize:** set `lane: done`. Merge the worktree branch back via the normal git workflow if used, then
